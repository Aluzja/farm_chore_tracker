...
<objective>
Create a mobile-first 7-day history view that shows completed chores with photos, timestamps, and completion info grouped by date.

Purpose: Users need to review past work - what was done, when, by whom, and with photo proof. This provides accountability and helps with farm management. The view will be used 80% on phones, so optimize for mobile scrolling and readability.

Output: Convex getHistory query, mobile-optimized history page with date-grouped display, navigation link from main UI.
</objective>
...
Styling (mobile-first - 80% phone use):
- Scoped CSS, no Tailwind
- Cards with subtle shadows for depth
- Photo thumbnails on right side of card (thumb-reachable)
- Touch-friendly spacing (min 56px row height for comfortable tapping)
- Sticky date headers stay visible while scrolling (position: sticky; top: 0)
- Full-width cards on mobile (no side margins wasted)
- max-width: 600px centered on desktop for readability
- Font sizes minimum 16px for readability outdoors
- Good contrast for bright sunlight visibility
- Use `-webkit-overflow-scrolling: touch` for smooth momentum scrolling
  </action>
  <verify>Navigate to /history on a phone (or mobile emulator), see completed chores grouped by date with smooth scrolling and readable cards.</verify>
  <done>History page displays 7 days of completed chores in a mobile-optimized layout with date grouping and photos.</done>
</task>
