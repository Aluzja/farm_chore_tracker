...
2. `capturePhoto(): Promise<{ blob: Blob; previewUrl: string } | null>` function:
   - Create hidden file input with:
     - type="file"
     - accept="image/*"
     - capture="environment" (prefer back camera)
   - Trigger click programmatically
   - On file selection, compress the image
   - Return compressed blob and object URL for preview
...
5. src/lib/db/schema.ts has DB_VERSION = 3 and PhotoQueueEntrySchema
6. src/lib/db/client.ts has photoQueue in KitchenSinkDB interface
</verification>
