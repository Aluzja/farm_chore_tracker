...
must_haves:
  ...
  key_links:
    ...
    - from: "src/routes/(app)/photo-capture/+page.svelte"
      to: "syncEngine.processPhotoQueue"
      via: "trigger photo upload after enqueue"
      pattern: "syncEngine\\.processPhotoQueue"
    - from: "src/routes/(app)/+page.svelte"
      to: "src/routes/(app)/photo-view/[id]/+page.svelte"
      via: "navigation on thumbnail tap"
      pattern: "photo-view"
    - from: "src/routes/(app)/+page.svelte"
      to: "api.photos.getPhotoUrl"
      via: "Convex query for thumbnail URLs"
      pattern: "useQuery.*getPhotoUrl"
---
...
Verify handleAccept calls both syncEngine.processQueue() AND syncEngine.processPhotoQueue().
  </verify>
  <done>
...
Explicitly triggers photo upload via syncEngine.processPhotoQueue() when online.
  </done>
</task>
...
<script lang="ts">
  ...
  const error = $derived(photoUrlQuery?.error?.message ?? null);
...
Verify it uses useQuery(api.photos.getPhotoUrl) for photo URL.
...
Uses Convex query for reactive photo URL fetching.
  </done>
</task>
...
2. Add photo thumbnail display next to completed chores that have photos:
   ...
   - Use Convex query to fetch photo URLs for thumbnails

For fetching thumbnail URLs, create a helper component or use useQuery directly:
```svelte
<script lang="ts">
  import { useQuery } from 'convex-svelte';
  import { api } from '../../../convex/_generated/api';
  ...
  // For each chore with a photoStorageId, query for the URL
  // Option A: Query per thumbnail (simple, fine for small lists)
  // Option B: Create a batch query in Convex (better for large lists)

  // Using Option A for simplicity - one query per thumbnail:
  function getPhotoThumbnailQuery(storageId: string | undefined) {
    if (!storageId) return null;
    return useQuery(api.photos.getPhotoUrl, { storageId: storageId as Id<"_storage"> });
  }
</script>

{#if chore.isCompleted && chore.photoStorageId}
  {@const photoQuery = getPhotoThumbnailQuery(chore.photoStorageId)}
  <button
  ...
  >
    {#if photoQuery?.data}
      <img src={photoQuery.data} alt="Completion photo" />
    {:else}
...
Verify photo thumbnails use useQuery(api.photos.getPhotoUrl) to fetch URLs.
...
Completed photos show as thumbnails using Convex queries for URLs.
...
8. Online photos upload automatically via syncEngine.processPhotoQueue()
</verification>
